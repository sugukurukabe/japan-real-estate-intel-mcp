...
describe('simulateAichiFuture — Linear Chuo Shinkansen', () => {
  it('returns high uplift for 名古屋市中村区 (direct Linear zone)', () => {
    const result = simulateAichiFuture({ city: '名古屋市中村区', scenarios: ['linear_chuo'], horizon: '10y', includeMarkdown: false });
  ...
  it('returns buy signal for 名古屋市中区 (wave effect)', () => {
    const result = simulateAichiFuture({ city: '名古屋市中区', scenarios: ['linear_chuo'], horizon: '10y', includeMarkdown: false });
...
describe('simulateAichiFuture — Centrair expansion', () => {
  it('returns positive uplift for 常滑市', () => {
    const result = simulateAichiFuture({ city: '常滑市', scenarios: ['centrair_expansion'], horizon: '10y', includeMarkdown: false });
...
describe('simulateAichiFuture — Toyota industrial', () => {
  it('returns positive uplift for 豊田市', () => {
    const result = simulateAichiFuture({ city: '豊田市', scenarios: ['toyota_industrial'], horizon: '10y', includeMarkdown: false });
  ...
  it('3y horizon returns lower uplift than 10y for same city', () => {
    const r3 = simulateAichiFuture({ city: '豊田市', scenarios: ['toyota_industrial'], horizon: '3y', includeMarkdown: false });
    const r10 = simulateAichiFuture({ city: '豊田市', scenarios: ['toyota_industrial'], horizon: '10y', includeMarkdown: false });
...
describe('simulateAichiFuture — signal thresholds', () => {
  ...
  it('compositeSignal reflects totalUpliftPct', () => {
    const result = simulateAichiFuture({ city: '名古屋市中村区', scenarios: ['all'], horizon: '10y', includeMarkdown: false });
...
describe('simulateAichiFuture — markdown output', () => {
  it('markdownReport is defined when includeMarkdown=true', () => {
    const result = simulateAichiFuture({ city: '名古屋市中村区', scenarios: ['all'], horizon: '10y', includeMarkdown: true });
  ...
  it('markdownReport is undefined when includeMarkdown=false', () => {
    const result = simulateAichiFuture({ city: '豊田市', scenarios: ['all'], horizon: '5y', includeMarkdown: false });
    expect(result.markdownReport).toBeUndefined();
  });

  it('markdownReport contains city name', () => {
    const result = simulateAichiFuture({ city: '豊田市', scenarios: ['all'], horizon: '10y', includeMarkdown: true });
...
describe('simulateAichiFuture — unknown city', () => {
  it('returns empty scenarios but does not throw for unknown city', () => {
    const result = simulateAichiFuture({ city: '架空市', scenarios: ['all'], horizon: '10y', includeMarkdown: true });
