...
export async function markdownToPdfBase64(
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    doc.fillColor('#ffffff').fontSize(16).font(F_BOLD)
      .text(branding?.companyName ?? 'Japan Real Estate Intel', 48, 20, { width: PAGE_W - 80 });

    if (branding?.agentName) {
      doc.fontSize(10).font(F_REG).fillColor('rgba(255,255,255,0.85)')
        .text(`\u62c5\u5f53: ${branding.agentName}`, 48, 44);
    }

    doc.fontSize(9).font(F_REG).fillColor('rgba(255,255,255,0.7)')
      .text(today, 48, 60);
    ...
    for (const raw of lines) {
      ...
      if (line.startsWith('## ')) {
        ...
        doc.fontSize(13).font(F_BOLD).fillColor(PRIMARY)
        ...
        doc.lineWidth(1);
        doc.fontSize(10).font(F_REG).fillColor(MUTED)
    ...
    if (comparables && comparables.length > 0) {
      ...
      doc.fillColor('#ffffff').fontSize(14).font(F_BOLD)
        .text('\u904e\u53bb\u53d6\u5f15\u4e8b\u4f8b\uff08\u53c2\u8003\uff09', 48, 16);
      doc.y = 70;

      doc.fontSize(11).font(F_BOLD).fillColor(TEXT)
        .text('\u8fd1\u96a3\u306e\u904e\u53bb\u6210\u7d04\u4e8b\u4f8b\uff08\u76f4\u8fd15\u5e74\uff09', { align: 'center' });
      doc.moveDown(0.5);

      const colWidths = [40, 55, 60, 55, 60, 55, 55, 55];
      const headers = ['\u5e74', '\u5e02\u533a', '\u5730\u533a', '\u7a2e\u5225', '\u9762\u7a4f(\u33a1)', '\u33a1\u5358\u4fa1(\u4e07)', '\u7bc9\u5e74', '\u69cb\u9020'];
      const startX = 48;
      let x = startX;
      const headerY = doc.y;
      ...
      doc.moveDown(0.5);
      doc.fontSize(8).font(F_REG).fillColor(MUTED)
    ...
    if (branding?.footerContact) {
      doc.moveDown(0.3);
      doc.fontSize(9).font(F_BOLD).fillColor(PRIMARY).text(branding.footerContact, { align: 'center' });
    }
