...
export function portfolioOptimizer(input: PortfolioOptimizerInput): PortfolioOptimizerOutput {
  ...
  const wRet = assets.reduce(
    (s, a, i) => s + a.expectedAnnualReturnPct * (allocations[i] / 100),
    0,
  );
  ...
  const divScore = Math.min(
    100,
    Math.round((prefKeys.size / targets.length) * 60 + (typeKeys.size / targets.length) * 40),
  );
  ...
  const topAsset = assets.reduce(
    (best, a) => (a.allocationPct > best.allocationPct ? a : best),
    assets[0],
  );
  ...
  const optLabel: Record<string, string> = {
    return: '最大リターン重視',
    risk_adjusted: 'リスク調整後リターン重視',
    diversification: '分散重視',
    stability: '安定性重視',
  };
    ...
    .map(
      (a, i) =>
        ...
        (a.currentPricePerSqm
          ? `- **参考地価**: ${a.currentPricePerSqm.toLocaleString()} 円/㎡\n`
          : ''),
    )
    .join('\n');
...
${
  portfolioReturn >= 4.5
    ? '積極的な収益性が期待できるポートフォリオです。'
    : portfolioReturn >= 3.5
      ? '堅実なリターンが見込まれる標準的なポートフォリオです。'
      : '保守的・安定性重視のポートフォリオです。'
}${
    divScore >= 70
      ? ' 地理的・物件種別の分散も良好です。'
      : divScore >= 40
        ? ' 分散は中程度です。追加エリアの検討も有効です。'
