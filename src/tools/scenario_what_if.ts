...
const SCENARIO_EFFECTS: Record<string, Record<string, ScenarioEffect>> = {
  new_commercial_facility: {
    small:  { priceImpactPct: 2.5,  humanFlowImpactPct:  8, riskImpactPct:  0, investmentScoreDelta:  3, label: '小規模商業施設の新規開業' },
    medium: { priceImpactPct: 6.0,  humanFlowImpactPct: 18, riskImpactPct:  0, investmentScoreDelta:  7, label: '中規模ショッピングセンター開業' },
    large:  { priceImpactPct: 12.0, humanFlowImpactPct: 35, riskImpactPct: -2, investmentScoreDelta: 12, label: '大型商業施設（ショッピングモール）開業' },
  },
  new_station: {
    small:  { priceImpactPct: 5.0,  humanFlowImpactPct: 15, riskImpactPct:  0, investmentScoreDelta:  5, label: '新駅設置（バス停・LRT）' },
    medium: { priceImpactPct: 10.0, humanFlowImpactPct: 25, riskImpactPct:  0, investmentScoreDelta: 10, label: '新駅設置（私鉄）' },
    large:  { priceImpactPct: 18.0, humanFlowImpactPct: 40, riskImpactPct: -2, investmentScoreDelta: 18, label: '新駅設置（JR・地下鉄）' },
  },
  new_corporate_office: {
    small:  { priceImpactPct: 2.0,  humanFlowImpactPct:  5, riskImpactPct:  0, investmentScoreDelta:  3, label: '中小企業オフィス新設' },
    medium: { priceImpactPct: 4.5,  humanFlowImpactPct: 10, riskImpactPct:  0, investmentScoreDelta:  6, label: '大企業支社・拠点新設' },
    large:  { priceImpactPct: 9.0,  humanFlowImpactPct: 22, riskImpactPct: -1, investmentScoreDelta: 11, label: '大規模オフィスビル・本社誘致' },
  },
  population_growth: {
    small:  { priceImpactPct: 1.5, humanFlowImpactPct:  3, riskImpactPct:  0, investmentScoreDelta:  2, label: '緩やかな人口増加（+2%）' },
    medium: { priceImpactPct: 4.0, humanFlowImpactPct:  8, riskImpactPct:  0, investmentScoreDelta:  5, label: '堅調な人口増加（+5%）' },
    large:  { priceImpactPct: 8.0, humanFlowImpactPct: 15, riskImpactPct: -2, investmentScoreDelta: 10, label: '急成長（+10%・大規模流入）' },
  },
  population_decline: {
    small:  { priceImpactPct: -2.0,  humanFlowImpactPct:  -4, riskImpactPct:  3, investmentScoreDelta:  -3, label: '緩やかな人口減少（-2%）' },
    medium: { priceImpactPct: -5.0,  humanFlowImpactPct: -10, riskImpactPct:  7, investmentScoreDelta:  -8, label: '中程度の人口減少（-5%）' },
    large:  { priceImpactPct: -12.0, humanFlowImpactPct: -20, riskImpactPct: 15, investmentScoreDelta: -18, label: '急激な人口流出（-10%以上）' },
  },
  disaster_risk_increase: {
    small:  { priceImpactPct: -3.0,  humanFlowImpactPct: null, riskImpactPct: 10, investmentScoreDelta:  -5, label: '軽微な災害リスク上昇（洪水ハザード見直し）' },
    medium: { priceImpactPct: -7.0,  humanFlowImpactPct: null, riskImpactPct: 20, investmentScoreDelta: -12, label: '中程度の災害リスク増大（震度想定見直し）' },
    large:  { priceImpactPct: -15.0, humanFlowImpactPct: null, riskImpactPct: 35, investmentScoreDelta: -25, label: '大規模災害リスク（直下型地震想定区域指定）' },
  },
  disaster_risk_decrease: {
    small:  { priceImpactPct: 1.5, humanFlowImpactPct: null, riskImpactPct: -5,  investmentScoreDelta:  3, label: '軽微なリスク低減（堤防整備）' },
    medium: { priceImpactPct: 3.5, humanFlowImpactPct: null, riskImpactPct: -12, investmentScoreDelta:  7, label: '中程度リスク低減（大規模治水事業）' },
    large:  { priceImpactPct: 8.0, humanFlowImpactPct: null, riskImpactPct: -25, investmentScoreDelta: 15, label: '大幅リスク軽減（地区防災計画・耐震化完了）' },
  },
};
...
export function scenarioWhatIf(input: ScenarioWhatIfInput): ScenarioWhatIfOutput {
  ...
  type SM = { pricePerSqm: number | null; humanFlowScore: number | null; investmentScore: number; riskScore: number };
  const baseline: SM = {
  ...
  const effect = SCENARIO_EFFECTS[input.scenario]?.[input.scale] ?? {
    priceImpactPct: 0, humanFlowImpactPct: null, riskImpactPct: 0, investmentScoreDelta: 0,
  ...
  const projected: SM = {
    pricePerSqm: baseline.pricePerSqm !== null
      ? Math.round(baseline.pricePerSqm * (1 + scaledPrice / 100))
      : null,
    humanFlowScore: baseline.humanFlowScore !== null && scaledHF !== null
  ...
  const confidence: 'high' | 'medium' | 'low' =
    effect.priceImpactPct !== 0 ? 'medium' : 'low';
  ...
  const recommendations = buildRecommendations(input.scenario, scaledPrice, baseline.investmentScore);

  let markdownReport: string | undefined;
  if (input.includeMarkdown) {
    markdownReport = buildMarkdown({
      input, effect, baseline, projected,
      scaledPrice, scaledRisk, scaledHF, confidence, opportunities, risks, recommendations,
    });
  }
...
function buildOpportunities(scenario: string, scale: string, priceDelta: number): string[] {
  const map: Record<string, string[]> = {
    new_commercial_facility: ['来街者増加による小売・飲食需要拡大', '周辺地価上昇による資産価値向上', '賃貸需要増加（住居・店舗）'],
    new_station: ['通勤利便性向上による居住需要増加', 'バス・タクシー等二次交通の整備促進', '駅前開発による商業集積機会'],
    new_corporate_office: ['オフィス需要増加', '従業員向け住宅・商業需要拡大', '法人税収増加による行政サービス向上'],
    population_growth: ['住宅・教育施設への旺盛な需要', '小売業・飲食業の市場拡大', '長期的な地価上昇基調'],
    population_decline: ['底値取得の機会（逆張り投資）', 'リノベーション・コンバージョン需要'],
    disaster_risk_increase: ['防災投資・耐震工事需要', '安全エリアへの移転コンサル需要'],
    disaster_risk_decrease: ['保険・融資条件の改善', '再開発・土地活用の障壁低減', '居住・商業需要の回帰'],
  };
...
function buildRisks(scenario: string, scale: string, priceDelta: number): string[] {
  const map: Record<string, string[]> = {
    ...
    population_decline: ['空き家増加・街の空洞化', '税収減による公共サービス低下', '流動性低下による出口戦略の困難化'],
    disaster_risk_increase: ['保険料上昇・融資条件悪化', '住宅・商業地の長期的価値低下', 'テナント・居住者の転出'],
...
function buildRecommendations(scenario: string, priceDelta: number, baseIS: number): string[] {
  ...
  if (baseIS < 50) recs.push('ベーススコアが低いため、シナリオ実現時の追加デューデリジェンスを推奨');
...
function buildMarkdown(opts: {
  ...
  const { input, effect, baseline, projected, scaledPrice, scaledRisk, scaledHF, confidence, opportunities, risks, recommendations } = opts;
