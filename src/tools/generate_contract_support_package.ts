...
export function generateContractSupportPackageTool(rawArgs: Record<string, unknown>): {
  ...
  const avgPricePerSqm = landPrices.length > 0
  ...
  const riskMatrix = [
    {
      clause: '洪水保険加入義務',
      riskLevel: (landPrices.some(p => p.land_use.includes('浸水')) || false) ? 'high' : 'medium' as const,
  ...
  const negotiationAnchors = [
    {
      ...
      recommendation: timeline.summary.avgPriceImpactPct > 8
        ? '今すぐ契約推奨（上昇局面）'
        : '様子見または価格交渉余地あり',
    },
    {
  ...
  const recommendedClauses = [
    ...
    },
    {
      ...
      priority: timeline.summary.avgPriceImpactPct > 10 ? 'recommended' as const : 'optional' as const,
    },
    {
      clause: 'リノベ費用超過上限',
  ...
  const summary = `名古屋市${input.ward}${input.chochou ? ' ' + input.chochou : ''} の売買契約支援レポートです。` +
  ...
  const markdown = [
    ...
    '',
    '## リスクマトリックス',
    ...riskMatrix.map(r => `- **${r.clause}** (${r.riskLevel})\n  - 理由: ${r.reason}\n  - 対策: ${r.mitigation}`),
    '',
    '## 価格交渉アンカー',
    ...negotiationAnchors.map(a => `- ${a.topic}\n  - 現在: ${a.currentPriceImpact}\n  - 将来: ${a.futureUplift}\n  - 判断: ${a.recommendation}`),
    '',
    '## 推奨特約',
    ...recommendedClauses.map(c => `- **[${c.priority.toUpperCase()}] ${c.clause}**\n  - ${c.rationale}`),
    '',
    '---',
